...
abstract class RedPokedexTheme {
  static ThemeData get pokedexTheme => ThemeData(useMaterial3: true);
}

enum RedPokedexTypeColor {
  ...
  const RedPokedexTypeColor(this.primary, this.secundary);

  final Color primary;
  final Color secundary;

  static RedPokedexTypeColor getPokemonColor(String type) => RedPokedexTypeColor.values.firstWhere(
        (element) => type.toLowerCase() == element.name,
        orElse: () => RedPokedexTypeColor.unknown,
      );
}
